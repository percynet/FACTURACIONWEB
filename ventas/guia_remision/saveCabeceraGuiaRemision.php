...
if(isset($_SESSION['paramdb']) && isset($_SESSION['USUARIO']) ){
	...
	$accion = $_POST["accion"];
    //echo "ACCION:".$accion;	
	$cabeceraJSON = $_POST["cabecera_guia_remision"];
	...
	if ($objdbSP -> is_connection_sp()){
		if($accion == "0"){
			$rsResultInsert = $objdbSP -> execSP_InsertCabeceraGuiaRemision($cabeceraJSON);
			...
			if($accion == "1"){
				$rsResultUpdate = $objdbSP -> execSP_UpdateCabeceraGuiaRemision($cabeceraJSON);
